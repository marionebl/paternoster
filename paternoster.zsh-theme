...
# Didr: current working directory
...
prompt_status() {
	local symbols
	symbols=()
	[[ $(jobs -l | wc -l) -gt 0 ]] && symbols="⚙" && prompt_segment black white "$symbols"
	[[ $RETVAL -ne 0 ]] && symbols="✘" && prompt_segment red white "$symbols"
	[[ $UID -eq 0 ]] && symbols="⚡" && prompt_segment yellow black "$symbols"
